...
async def initialize_storage() -> None:
    ...
    try:
        ...
        storage_type = os.environ.get("STORAGE_TYPE", "mongodb")
        logging.info(
            f"Knowledge Graph MCP Server initialized with {storage_type} storage"
        )
...
@mcp.tool()
async def find_path(
    start_entity: str, end_entity: str, max_depth: int = 5
) -> List[Entity]:
